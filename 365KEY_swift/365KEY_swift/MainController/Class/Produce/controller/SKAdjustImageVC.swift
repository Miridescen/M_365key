//
//  SKAdjustImageVC.swift
//  365KEY_swift
//
//  Created by 牟松 on 2016/12/27.
//  Copyright © 2016年 DoNews. All rights reserved.
//  本控制器用于从相册选择好图片后，对图片剪裁用的控制器

import UIKit
public protocol SKAdjustImageVCDelegate: NSObjectProtocol{
    func getImageSuccess(image: UIImage)->Void
}
class SKAdjustImageVC: UIViewController {
    ...
    weak var delegate: SKAdjustImageVCDelegate?
    ...
    @objc private func sureBtnDidclick(){
        ...
        dismiss(animated: true){
            self.delegate?.getImageSuccess(image: self.getsubImage(image: lastImage!, rect: CGRect(x: 0, y: (SKScreenHeight-64-SKScreenWidth*140/238)/2, width: SKScreenWidth, height: SKScreenWidth*140/238-64)))
    ...
    func setSubview() {
        ...
        topView = UIView(frame: CGRect(x: 0, y: 0, width: SKScreenWidth, height: (SKScreenHeight-64-SKScreenWidth*140/238)/2))
        topView?.backgroundColor = UIColor(white: 0.5, alpha: 0.6)
        view.addSubview(topView!)
        
        
        bottemView = UIView(frame: CGRect(x: 0, y: (SKScreenHeight-64-SKScreenWidth*140/238)/2+SKScreenWidth*140/238-64, width: SKScreenWidth, height: SKScreenHeight-(SKScreenHeight-64-SKScreenWidth*140/238)/2-SKScreenWidth*140/238-70))
...
extension SKAdjustImageVC: UIGestureRecognizerDelegate{
